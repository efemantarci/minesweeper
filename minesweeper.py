...
#No root window hatası yediğim için buraya çektim sonra düzelticem
#13 tane ile başlatıyorum hardcodeladık biraz
images = [None for i in range(14)]
...
def finish():
    for y in range(HEIGHT):
        for x in range(WIDTH):
            #Yanlış flag varsa
            if buttons[y][x]["text"] == BOMB and not bomb_grid[y][x]:
                buttons[y][x].configure(image=images[13])
